...
export default function Allmasters() {
  ...
  const [Ltrade, setLtrade] = React.useState([]);
  ...
  const getLastTradeDetails = async () => {
    await Axios.get(`${Consts.BackendUrl}/trade/last_trade_history`, {
      headers: {
        Authorization: localStorage.getItem("Mellifluous"),
      },
    })
      .then((res) => setLtrade(res.data.result))
            ...
            <TableBody>
              {masters &&
                masters.map((row, index) => {
                  if(Ltrade?.length > 0 ){
                    console.log("one")
                    for (let a = 0; a < Ltrade?.length; a++) {

                      if (Ltrade[a]?.user_id == row?.master._id) {
                      ...
                      return   <TableRow
                        ...
                        >
                          ...
                          <TableCell align="center">
                            <div>
                              <span style={{ color: "#25DEB0" }}>
                              {((Number(Ltrade[a]?.exit_price) - Number(Ltrade[a]?.entry_price)) / Math.abs(Number(Ltrade[a]?.entry_price))) * 100 > 0  ? `${(((Number(Ltrade[a]?.exit_price) - Number(Ltrade[a]?.entry_price)) / Math.abs(Number(Ltrade[a]?.entry_price))) * 100).toFixed(2)} %`:"-"}
                              </span>
                          ...
                          <TableCell align="center">
                            <div>
                              ...
                              <Link
                                to={`${Constant.route}/statergy-list/${row?.master._id}`}
                              >
                                <Button variant="contained">Details</Button>
                              </Link>
                              {/* <Link to={`${Constant.route}/trader-details/${row.user._id}`} ><Button variant="contained">Details</Button></Link> */}
                            </div>
                          </TableCell>
                        </TableRow>;
                      } else {
