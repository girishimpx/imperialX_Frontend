...
const ReferralsBody = () => {
  ...
  const handleCopy = async (cbdata) => {
    try {
      await navigator.clipboard.writeText(cbdata);
      ...
      // setCopyStatus('Referral code copied!');
  ...
  const handleEmailClick = (link) => {
    const subject = encodeURIComponent('Invite From Imperial');
    const body = encodeURIComponent(`Hello,\n\n${user?.result?.name} Invited You To ImperialX-exchange\n\nPlease refer to the link below for register:\n\n${link}\n\nThank you.`);
            ...
            <Grid container spacing={0} className={classes.invitefriendsoutercontainer}>
              <Grid item xs={12} sm={12} md={12} lg={8} xl={8} className={classes.invitefriendsouter}>
                <div className={classes.invitefriendsleft}>
                  <h5>Your Refferal Code : {user?.result?.referral_code}</h5>
              ...
              <Grid item xs={12} sm={12} md={12} lg={4} xl={4} id="invitefriendsright">
                ...
                <div className='referal-code-link-outer'>
                  ...
                  <div className='My-Commission'>
                    <label>Referral Code</label>  <span>{user?.result?.referral_code} {!loading ? <ContentCopyIcon style={{ cursor: 'pointer' }} onClick={() => handleCopy(user?.result?.referral_code)} /> : null} </span>
                  </div>
                  {/* <div className='refer-link'> */}
                  <div className='My-Commission'>
                    {/* <label>Referral Link</label>  <span> { `${Consts?.MainUrl+'/'+register+'/'+user?.result?.referral_code}`.slice(0,10) } <ContentCopyIcon style={{ cursor: 'pointer'}} onClick={() => { handleCopy(`${Consts?.MainUrl}`+'/'+`${register}`+'/'+`${user?.result?.referral_code}`)} }/> </span> */}
                    <label>Referral Link</label>  <span> {'Copy Link'}
                      {!loading ? <ContentCopyIcon style={{ cursor: 'pointer' }} onClick={() => { handleCopy(`${Consts?.MainUrl}` + '/' + `${register}` + '/' + `${user?.result?.referral_code}`) }} /> : null}  </span>
                  </div>
                  <Button className='invite-friends-button' disabled={loading} onClick={() => { handleEmailClick(`${Consts?.MainUrl}` + '/' + `${register}` + '/' + `${user?.result?.referral_code}`); }} variant="contained" endIcon={<ArrowForwardIcon />}> {'Invite Friends'}</Button>
                </div>
              </Grid>
            </Grid>
